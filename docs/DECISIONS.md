...
| D-021 | 2026-07-18 | Değiştirildi | Extraction şeması v0.1, gerçek Türkçe fatura örnekleriyle doğrulanmadan DONDURULMAZ (DRAFT). (D-058 ile değiştirildi.) | Şema donmadan ground-truth etiketleme yapılırsa, şema değişince etiketler çöpe gider; freeze ayrı bir şema review adımında verilecektir. | 2026-07-23'te 3 kapsam içi gerçek belge üzerinde anonim review tamamlandı ve freeze kararı verildi; bkz. D-058. |
...
| D-031 | 2026-07-20 | Değiştirildi | Schema freeze, extraction accuracy'ye değil dört temsil edilebilirlik kriterine dayanır (temsil edilebilirlik, çözülmemiş boşluk yok, format kapsamı, LineItem yeterliliği). Freeze için **en az 10** temsilî gerçek fatura gerekir. (Sayı hedefi D-058 ile değiştirildi; dört kriter geçerliliğini korur.) | Şema doğru temsil ediyorsa düşük ölçülmüş doğrulukla bile dondurulabilir; accuracy'ye bağlamak yanlış bir kapı olur. Coverage matrisi yöntemdir, ayrı kriter değil. | 10 belgelik eşik pratikte ulaşılamadı (erişilebilir kapsam içi gerçek belge sayısı 3'te kaldı); bkz. D-058. |
...
| D-058 | 2026-07-23 | Kabul edildi | Extraction şeması v0.1, **3** kapsam içi gerçek belge üzerinde gözlenen representability temelinde DONDURULDU (D-021 ve D-031'in 10 belgelik sayı hedefi yerine geçer). Freeze yalnızca dar V1.0 kapsamını sabitler; accuracy iddiası taşımaz ve N=3 dış geçerlilik sağlamaz. | Erişilebilir gerçek belge kaynağı pratikte 3 ile sınırlı kaldı; 10–12 hedefi uygulanabilir değildi ve şemayı süresiz DRAFT tutmak ground-truth etiketlemeyi ve evaluation'ı kilitliyordu. D-031'in dört temsil edilebilirlik kriteri gözlenen üç belgede sağlandı: dokuz header ve beş LineItem alanının tamamı mevcut, okunabilir ve kayıpsız temsil edilebilir; hiçbir değer `unreadable`'a düşmedi; yeni zorunlu alan gerekmedi. Sentetik set freeze veya accuracy kanıtı değildir. | Yeni gerçek belgeler toplandığında (özellikle çok satırlı, çoklu KDV oranlı, iskontolu veya farklı layout ailesinden) schema v0.2 gap analizi başlatılır. |
| D-059 | 2026-07-23 | Kabul edildi | Satır aritmetiğinde (ARITH-003) tolerance v0.1'de tanımlanmaz; karşılaştırmalar tam `Decimal` eşitliği olarak kalır. Gerçek veride yuvarlama kaynaklı false positive **gözlenmiştir** (1/3). | Tolerance eşiği keyfi seçilirse gerçek çelişkileri de gizler; doğru eşik ancak daha fazla gerçek belge görüldüğünde belirlenebilir. Gözlemi kayıt altına alıp kuralı değiştirmemek, bulgunun kaybolmasını önler ve insan denetimi zaten `blocking` sinyali görüp karar verebilir. | Tolerance politikası ruleset 0.2'de ayrı bir karar olarak ele alınacaktır; birden fazla gerçek belgede aynı false positive görülürse öncelik kazanır. |
